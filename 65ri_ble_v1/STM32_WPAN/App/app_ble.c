...
uint8_t a_AdvData[26] =
{
  ...
  6, AD_TYPE_COMPLETE_LOCAL_NAME, 'W', 'B', 'A', '6', '5',  /* Complete name */
...
uint8_t peer_addr[6] = {0x71, 0x98, 0x69, 0xC5, 0xB0, 0xE8}; // ✅ 反序的笔记本网卡地址
/* USER CODE END PFP */
...
void APP_BLE_Init(void)
{
  ...
  if (HOST_BLE_Init() == 0u)
  {
    ...
    uint8_t peer_addr[6] = {0x71, 0x98, 0x69, 0xC5, 0xB0, 0xE8}; // 反序
    ...
    /* Start to Advertise to accept a connection */
          ...
          //     ADV_DIRECT_IND,             // paramD 改过的值
          //     paramA,                     // interval min
          //     paramB,                     // interval max
          //     CFG_BD_ADDRESS_TYPE,        // own address type
          //     ADV_FILTER,                 // advertising filter
          //     GAP_PUBLIC_ADDR,            // ✅ PeerAddrType
          //     peer_addr,                  // ✅ PeerAddr
          //     0, 0, 0, 0);                                        // 放置位置 APP_BLE_Procedure_Gap_Peripheral，switch(ProcGapPeripheralId)
    APP_BLE_Procedure_Gap_Peripheral(PROC_GAP_PERIPH_ADVERTISE_START_FAST);  // 普通广播
    /* 启动一个超时定时器可选 */
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE:
    {
      ...
      paramA = CONN_INT_MS(10);  // 最快 7.5 ms
      paramB = CONN_INT_MS(10);
      paramC = 0x0000;            // 无延迟
